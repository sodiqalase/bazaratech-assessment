	...
	return (
		<div className="flex-shrink-0">
			<Popover.Root open={open && !disabled} onOpenChange={setOpen}>
				...
				{options && (
					<Popover.Portal>
						<Popover.Positioner sideOffset={10} align={"right" === "right" ? "end" : "start"}>
							{hasVisibleOption && (
								<Popover.Popup
									className={cn(
										"bg-white rounded-[4px] overflow-hidden shadow-[0px_2px_20px_rgba(0,0,0,0.13)] flex flex-col min-w-[10rem]",
										dropDownClass
									)}
								>
									{options
										...
										.map((option, index) => {
											return (
												<button
													...
													className={cn(
														"text-left pl-4 pr-4 py-2 text-xs group",
														"min-w-[6rem] font-normal flex items-center",
														"w-full truncate flex-shrink-0 block text-rp-grey-disabled",
														option.disabled && "cursor-not-allowed opacity-50"
													)}
													onClick={() => {
														if (option !== undefined && option.action) {
															if (!option.remainOpenOnClick) option.action();
															if (option.remainOpenOnClick) return option.action();
														}
														setOpen(false);
													}}
												>
													<span className="text-sm text-baz-secondaryText">
														{option.name}
													</span>
												</button>
											);
