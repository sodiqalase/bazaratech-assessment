...
interface ExternalLinkProps extends React.AnchorHTMLAttributes<HTMLAnchorElement>, CommonProps {
...
const Button = forwardRef<HTMLButtonElement | HTMLAnchorElement, Props>((props, ref) => {
	...
	if (props.renderAs === "external_link") {
		...
		return (
			<a
				...
				onClick={disabled ? (e: MouseEvent) => e.preventDefault() : anchorProps.onClick}
				{...anchorProps}
			>
				{_children}
			</a>
	...
	return (
		...
		>
			...
			<span className={cn("flex gap-x-3 items-center justify-center", loading && "opacity-50")}>{_children}</span>
		</button>
	);
});
